...
import sys
import requests
from multiprocessing import Pool  # <-- Added import for multiprocessing
...
def process_single_file(args):
    """
    Encapsulates the per-file logic previously in the main for-loop.
    Returns a tuple of:
        ( filename,
          full_sequence,
          log_text,         # Detailed log string to write
          is_incomplete )   # Boolean: True if missing residues were found
    """
    (filename, input_dir, pdb_suffix, cache_dir) = args

    # Prepare a local "log_output" that accumulates all log text instead of writing immediately.
    log_output = []
    ...
    if chain_id is None:
        ...
        log_output.append(f"File: {filename}\n")
        log_output.append("Error: No ATOM records found to determine chain ID.\n")
        log_output.append("=" * 60 + "\n\n")
        # Return early—no sequence.
        return (filename, "", "".join(log_output), False)
    ...
    if full_pdb_lines is None:
        log_output.append("=" * 60 + "\n")
        log_output.append(f"File: {filename}\n")
        log_output.append(f"Error: Failed to download full PDB for {pdb_id}.\n")
        log_output.append("=" * 60 + "\n\n")
        return (filename, "", "".join(log_output), False)
    ...
    is_incomplete = bool(diff_keys)

    # Build the detailed log information:
    log_output.append("=" * 60 + "\n")
    log_output.append(f"Processing file: {filename}\n")
    log_output.append("-" * 60 + "\n")
    log_output.append(f"PDB ID: {pdb_id}\n")
    log_output.append(f"Chain ID: {chain_id if chain_id else '[blank]'}\n")
    log_output.append("\nResolved Residues (from ATOM records):\n")
    for k in resolved_keys:
        res_info = resolved[k]
        log_output.append(f"  Residue {k[0]}{k[1] if k[1] else ''}: {res_info[0]} -> {res_info[1]}\n")
    ...
    if missing:
        ...
        log_output.append("  None\n")

    log_output.append("\nReconstructed Full Sequence:\n")
    log_output.append(f"  {full_sequence}\n")
    log_output.append("\nDifferences (Missing residues inserted):\n")
    if diff_keys:
        for k in diff_keys:
            if k in missing:
                res_info = missing[k]
                log_output.append(f"  Inserted missing residue at position {k[0]}{k[1] if k[1] else ''}: {res_info[0]} -> {res_info[1]}\n")
    else:
        log_output.append("  None (Resolved sequence is complete)\n")
    log_output.append("=" * 60 + "\n\n")

    return (filename, full_sequence, "".join(log_output), is_incomplete)
...
def main():
    ...
    # Collect all filenames to process
    files_to_process = []
    ...
    # Open output files and log file once in the main process.
    with open(log_file, "w") as log_f, \
        ...
        # Create a Pool to process files in parallel
        with Pool() as pool:
        ...
        # Now, write results to the output files (TSVs and log)
        for (filename, full_sequence, log_text, is_incomplete) in results:
            # If something prevented processing, we might have an empty sequence/log
            if not full_sequence and "Error:" in log_text:
                # Just write out the log error and continue
